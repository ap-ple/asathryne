...
class Character():
  ...
  def __init__(self, name, hp, ap, lvl, strength, intelligence, agility, defence, abilities = [], inv = [], gp = 0):
...
class PlayerCharacter(Character):
  ...
  def __init__(self, name, class_type, hp, ap, lvl, strength, intelligence, agility, defence, abilities = [], inv = [], gp = 0, xp = 0, abi_points = 0):

    Character.__init__(self, name, hp, ap, lvl, strength, intelligence, agility, defence, abilities = [], inv = [], gp = 0)
  ...
  def item_find(self, item):
    ...
    while True:
      ...
      if x == "L":
        ...
        self.gp += int(0.7 * item.value) #if you leave behind an item, you will recieve 70% of its value in gold
        return
      else:
...
"""
character
hp - How much damage the character can take before they perish
ap - Determines the character's use of abilities
...
inv - list of items the character carries
...
pot_heath = Item("Heath Potion", 20, consumable = True)
...
def Sanctuary_Apothecary():
  ...
  while True:
    print(f"--- You have {player.gp} gold.")
